...
async def on_message(message):
	...
	if message.author != client.user:
		...
		if not safe:
			for a in banWords:
					...
					if ' ' + a + ' ' in content or content.startswith(a) or content.endswith(a):
						banned_word = True
		has_link = any(url in message.content for url in ['discord.gg/', 'discordapp.com/invite/'])
		if is_repeating(message.content, phrase_limit) or banned_word or has_link:
			...
			await write('spamChart', sc)
			if banned_word:
				msg = await channel.send(f'<@!{user.id}> That word is not allowed!')
			elif has_link:
				msg = await channel.send(f'<@!{user.id}> invite links are not allowed!')
			else:
				msg = await channel.send(f'<@!{user.id}> please dont spam!')
		...
		if offenseLimit - 1 == spamChart[guildId][userId]:
			msg = await channel.send(f'<@!{str(userId)}> please stop spamming. You have been warned')
