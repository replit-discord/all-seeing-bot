...
class Customization(commands.Cog, name='customization'):
    ...
    @commands.command(name='strikelimit', aliases=['sl'])
    async def strike_max(self, ctx, ammount: int = None):
        '''Set the max strike count before a user is banned.'''
        guild = ctx.guild
        author = ctx.author
        strike_limits = await read('sl')

        if ammount is not None:
            strike_limits[guild.id] = ammount
            await ctx.send(f'Strike limit has been set to `{ammount}`.')
            await self.log(
                ctx,
                f'<@{author.id}> set the strike limit to `{ammount}`.'
            )
            await write('sl', strike_limits)
        else:
            if guild.id in strike_limits:
                current_limit = strike_limits[guild.id]
                await ctx.send(f'The current strike limit is `{current_limit}')
            else:
                await ctx.send('The strike limit is not set!')

    @commands.command(name='warnlimit', aliases=['wl'])
    async def warn_limit(self, ctx, ammount: int = None):
        '''Set the max strike count before a user is banned.'''
        guild = ctx.guild
        author = ctx.author
        warn_limits = await read('wl')

        if ammount is not None:
            warn_limits[guild.id] = ammount
            await ctx.send(f'Warn limit has been set to `{ammount}`.')
            await self.log(
                ctx,
                f'<@{author.id}> set the warn limit to `{ammount}`.'
            )
            await write('wl', warn_limits)
        else:
            if guild.id in warn_limits:
                current_limit = warn_limits[guild.id]
                await ctx.send(f'The current warn limit is `{current_limit}')
            else:
                await ctx.send('The warn limit is not set!')
